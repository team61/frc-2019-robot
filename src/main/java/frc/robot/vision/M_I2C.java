...
//            System.out.println("NO DATA FROM ARDIUNO");
            pkt.setX(-1); //the x val will never be -1 so we can text later in code to make sure
            //there is data
            pkt.setY(-1);
            pkt.setWidth(-1);
        }else if(info.length == 3){//if there is an x, y, and area value the length equals 3
//            System.out.println("DATA FROM ARDUINO");
            pkt.setX(Double.parseDouble(info[0]));//set x
            pkt.setY(Double.parseDouble(info[1]));//set y
            pkt.setWidth(Double.parseDouble(info[2]));//set Width
        }
        return pkt;
    }
